...
from PyQt5.QtWidgets import QMessageBox, QWidget, QGroupBox, QLabel, QLineEdit, QRadioButton, QTextEdit, QPushButton, QApplication
...
class Ui_frmVeiculos(object):
    ...
    # VERIFICA O ESTADO INSERIR/ALTERAR PARA CHAMAR A FUNÇÃO APROPRIADA
        if estado == 'inserir':
            veiculo = VeiculoCTR()
            veiculo.CadastrarVeiculo(codigoVeic, modelo, marca, anoModelo, placa, alugado, batido, kmAtual, valorDiaria, descricao, tipoVeiculo)
        ...
        if estado == 'alterar':
            veiculo = VeiculoCTR()
            veiculo.AtualizarVeiculo(codigoVeic, modelo, marca, anoModelo, placa, alugado, batido, kmAtual, valorDiaria, descricao, tipoVeiculo)
...
if __name__ == "__main__":
    ...
    frmVeiculos = QWidget()
    ui = Ui_frmVeiculos()
    ui.setupUi(frmVeiculos, 'inserir', 0)  # Substitua 'inserir' e 0 pelos valores desejados
    frmVeiculos.show()
    sys.exit(app.exec_())
